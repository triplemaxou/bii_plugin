...
class PDML extends FPDF {
	...
	function Ellipse($x, $y, $rx, $ry, $style = 'D',$astart = 0, $astop = 360) {
		if ($style == 'F')
			$op = 'f';
...
//		$this->Ellipse2($x, $y, $rx, $ry, $angle, $astart, $afinish, $style, $line_style, $fill_color);
	}
    ...
    function Ellipse2($x0, $y0, $rx, $ry = 0, $angle = 0, $astart = 0, $afinish = 360, $style = 'D', $line_style = null, $fill_color = null, $nSeg = 8) {
	...
	function RoundedRect($x, $y, $w, $h, $r = 0, $round_corner = '0000', $style = '', $border_style = null, $fill_color = null) {
			...
			if ($i % 2 == 0) {
				...
				if ($e{0} == '/') {
					...
					for ($i = 0; $i < count($matches); $i++) {
						$val = $matches[$i][2];
						if ((($val{0} == '"') and ( substr($val, -1) == '"')) or ( ($val{0} == "'") and ( substr($val, -1) == "'"))) {
...
//				break;
			...
			case "RECT":
				...
				if (isset($attr["RADIUS"])) {
					$radius = $this->_getUnit($attr["RADIUS"]);
				}
				if (isset($attr["ROUNDED"])) {
					$rounded = strtolower(strtoupper($attr["ROUNDED"]));
					if($rounded == "all" || $rounded == "a"){
						$round_corner = "1111";
					}
					elseif($rounded == "bottom" || $rounded == "b" ){
						$round_corner = "0011";
					}
					elseif($rounded == "top" || $rounded == "t" ){
						$round_corner = "1100";
					}
					elseif($rounded == "left" || $rounded == "l" ){
						$round_corner = "1010";
					}
					elseif($rounded == "right" || $rounded == "r" ){
						$round_corner = "0101";
					}
					else{
						$round_corner = 0;
						if(strpos($rounded, "tl") !== false || strpos($rounded, "lt") !== false){
							$round_corner += 100;
						}
						if(strpos($rounded, "tr") !== false || strpos($rounded, "rt") !== false){
							$round_corner += 1000;
						}
						if(strpos($rounded, "bl") !== false || strpos($rounded, "lb") !== false){
							$round_corner += 10;
						}
						if(strpos($rounded, "br") !== false || strpos($rounded, "rb") !== false){
							$round_corner += 1;
						}
						$round_corner = str_pad($round_corner, 4, "0", STR_PAD_LEFT);
					}
...
//				$this->Rect($x1, $y1, $width, $height, $style);
				...
				$this->RoundedRect($x1, $y1, $width, $height, $radius, $round_corner, $style);
				break;
			case "TRIANGLETITLE":
				$this->_enforceState(50, 50);
				// auto-create a page if needed.
				...
				$this->Polygon(array($depx, $depy, $pt2x, $pt2y, $pt3x, $pt3y, $pt4x, $pt4y, $pt5x, $pt5y), 'F');
			...
			case "TRIANGLERECTANGLE":
				...
				$this->Polygon(array($depx, $depy, $pt2x, $pt2y, $pt3x, $pt3y), 'F');
			...
			case "ELLIPSE":
				...
				$this->SetLineWidth($border);
				$this->_setRectColor($fill);
				$this->Ellipse2($x1, $y1, $xradius, $yradius, $angle, $astart, $astop, $style);
				break;
			case "DIV":
				$this->_enforceState(50, 50);
				// auto-create a page if needed.
...
//				break;
			...
			case "MULTICELL":
				...
				$this->MultiCell(
					$this->cell_info[0], $this->cell_info[1], $this->cell_text, $this->cell_info[3], $this->cell_info[4], $this->cell_info[5]);
				break;
			case "CELL":
				$this->_enforceState(51, 50);
				...
				if (sizeof($this->href) > 0) {
					...
					$this->Cell(
						$this->cell_info[0], $this->cell_info[2], $this->cell_text, $this->cell_info[3], $this->cell_info[6], $this->cell_info[4], $this->cell_info[5], $this->href[0]);
					if ($this->href_style[0]) {
						$this->_setStyle('U', $this->U > 0, true);
						$this->_setFontColor($this->font_color[0]);
					}
					...
					$this->Cell(
						$this->cell_info[0], $this->cell_info[2], $this->cell_text, $this->cell_info[3], $this->cell_info[6], $this->cell_info[4], $this->cell_info[5]);
				}
				break;
			case "ROTATE":
				$this->rotate(0);
			...
			case "BARCODE":
				$this->_enforceState(51, 50);
				$this->_Code39(
					$this->cell_info[0], $this->cell_info[2], $this->cell_text, $this->cell_info[6], $this->cell_info[3], $this->cell_info[7]);
				break;
		}
	}
...
//	$nom_fichier = "TDB-".$client->nom()."-$mois-$annee";
	$nom_fichier = "test.pdf";

	$pdml = new PDML($PDML_Orientation, 'pt', $PDML_Format);
	
	$pdml->AddFont('Din','','DINAlternateMedium.php');
	$pdml->AddFont('DinBold','','DINAlternateBold.php');
	
	$pdml->AddFont('Roboto','','Roboto-Medium.php');
	$pdml->AddFont('RobotoBold','','Roboto-Bold.php');
	$pdml->AddFont('RobotoThin','','Roboto-Thin.php');
	
	$pdml->AddFont('Calibri','','calibri.php');
	$pdml->AddFont('CalibriBold','','calibrib.php');
	$pdml->AddFont('CalibriItalic','','calibrii.php');
	$pdml->AddFont('CalibriBoldItalic','','calibriz.php');
	
	$pdml->compress = 0;
	$pdml->ParsePDML($buffer);
//	$pdml->Output($nom_fichier, 'F');
//		$pdml->Output("$nom_fichier", "I");
	$s = $pdml->Output("$nom_fichier", "S");
	

	
	
//	global $PDML_FileName;
////	$myfile = fopen(dirname(__FILE__) . "/archivestdb/test.txt", "w+");
...
function ob_pdml($buffer) {
	global $PDML_FileName, $PDML_Content_disposition;

	if($PDML_Content_disposition != "attachement"){
		$PDML_Content_disposition = "inline";
	}
...
//	fclose($myfile);
	...
	Header('Content-disposition: '.$PDML_Content_disposition.'; filename=' . $PDML_FileName);
...
function pdml_entity_decode($given_html, $quote_style = ENT_QUOTES) {
	$trans_table = array_flip(array_merge(
			get_html_translation_table(HTML_SPECIALCHARS, $quote_style), get_html_translation_table(HTML_ENTITIES, $quote_style)));
	$trans_table['&#39;'] = "'";

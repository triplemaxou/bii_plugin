...
class bii_item_page_perso extends global_class {
	...
	protected function call_lang($methode, $lang) {
		$var = "$methode" . "_$lang";
		return $this->$var();
	}
	...
	protected static function liste_mois($lang = "fr") {
		...
		if ($lang == "en") {
			$mois = array("", "January", "Febuary", "Marth", "April", "May", "June", "July", "August", "September", "October", "November", "December");
		}
		return $mois;
	}
...
//</editor-fold>
	...
	function front_titreEdit() {
		return $this->titreEdit();
	}

	function getSomething($where, $class, $type_return = "array_id") {
		$liste_item = $class::all_id($where);
		if ($type_return == "array_id") {
			return $liste_item;
		}
		if ($type_return == "array_items") {
			$listeitems = [];
			foreach ($liste_item as $id) {
				$listeitems[] = new $class($id);
			}
			return $listeitems;
		}
	}

	public static function front_redirectStay() {
		return um_user_profile_url() . "?profiletab=page_pro_edit";
	}

	public static function front_redirectEdit() {		
		$page_perso = bii_page_perso::get_page_perso();
		return get_permalink($page_perso->post_id());
	}

	public static function front_redirectError() {
		return static::front_redirectStay();
	}

...
var multiselectapplied = false;
function applymultiselect(selector) {
	if (!multiselectapplied) {
		jQuery(selector).multiSelect({
			selectionHeader: "<span>Afficher pour ces utilisateurs</span>",
			selectableHeader: "<span>Ne pas afficher pour ces utilisateurs</span>"
		});
	}
	multiselectapplied = true;
}

if (jQuery("#bii_userestrict_content").val() == 1) {
	jQuery(".bii_preloader").removeClass("hidden");
	applymultiselect(".multiselect");
...
jQuery(".cbx-data-change").on("click", function () {
	...
	if (checked == true) {
		...
		jQuery(".bii_preloader").addClass("hidden");
	}
